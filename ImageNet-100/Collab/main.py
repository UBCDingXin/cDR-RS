...
def delta_size(batch_size, gan_net=args.gan_net):
    if gan_net=="BigGAN":
        return (batch_size, 384, 32, 32)
    elif gan_net=="BigGANdeep":
        return (batch_size, 512, 32, 32)
    else:
        raise Exception("Not supported GAN!!")
    ...
    if not os.path.isfile(filename_netD_ckpt):
        ...
        for i in pbar:
            ...
            for k in range(args.rollout_steps):
                optim_r.zero_grad()
                batch_fake_images = netG(noise_batch, netG.module.shared(batch_labels), delta_refine) ##delta_refine is added to a hidden map of G
            ...
            # train with refined
            adv_label.fill_(0)
            batch_fake_images = netG(noise_batch, netG.module.shared(batch_labels), proba_refine)
            output = netD(batch_fake_images.detach(), batch_labels)
...
def collab_sampling(given_label, netG, netD, nfake=10000, batch_size=100, verbose=True):
    ...
    while num_taken < nfake:
        ...
        for k in range(args.rollout_steps):
            optim_r.zero_grad()
            batch_fake_images = (netG(z, netG.module.shared(batch_labels), delta_refine)).detach()
            output = netD(batch_fake_images, batch_labels)
            loss_r = criterion(output.view(-1,1), avd_label.view(-1,1))
            loss_r.backward()
            optim_r.step()
        batch_fake_images = (netG(z, netG.module.shared(batch_labels), delta_refine)).detach().cpu().numpy()
