...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.keep_training_batchsize, shuffle=True)
...
if args.gan_net=="BigGANdeep":
    ...
    netG = nn.DataParallel(netG)
    netD = SAGAN_Discriminator(num_classes=args.num_classes)
...
if args.keep_training:
    ...
    filename_netD_keeptrain = os.path.join(save_models_folder, 'ckpt_netD_keepTrain_epochs_{}.pth'.format(args.keep_training_epochs))
    if not os.path.isfile(filename_netD_keeptrain):
        netD = finetune_netD(trainloader, netG=netG, netD=netD)
        # save model
        torch.save({
...
def fn_sampleGAN_given_label(nfake, given_label, batch_size, pretrained_netG=netG, to_numpy=True):
    '''
    Sample from a pre-trained cGAN given a fixed label as condition. 
    nfake: number of fake images per label
    '''
    raw_fake_images = []
    raw_fake_labels = []
    ...
    with torch.no_grad():
        ...
        while num_taken < nfake:
            ...
            num_taken += batch_size

    raw_fake_images = torch.cat(raw_fake_images, dim=0)
    raw_fake_labels = (given_label*torch.ones(len(raw_fake_images))).type(torch.long).view(-1)
...
def comp_cond_density_ratio(imgs, labels, netD=netD, batch_size = args.samp_batch_size):
    ...
    with torch.no_grad():
        ...
        while n_imgs_got < n_imgs:
            ...
            disc_scores_exp = np.exp(netD(batch_images, batch_labels).cpu().detach().numpy())
            disc_scores_exp = np.clip(disc_scores_exp.astype(float), a_min=1e-14, a_max=None)
            density_ratios.append(disc_scores_exp)
