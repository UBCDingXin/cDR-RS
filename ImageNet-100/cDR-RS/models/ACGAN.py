...
class ACGAN_Generator(nn.Module):
    def __init__(self, nz=128, ny=100):
        super(ACGAN_Generator, self).__init__()
        self.dim_z = nz

        self.dense = nn.Linear(self.dim_z, 4 * 4 * GEN_SIZE*16, bias=True)
        self.final = nn.Conv2d(GEN_SIZE, channels, 3, stride=1, padding=1)
        nn.init.xavier_uniform_(self.dense.weight.data, 1.)
        nn.init.xavier_uniform_(self.final.weight.data, 1.)

        self.genblock1 = ResBlockGenerator(GEN_SIZE*16, GEN_SIZE*10, num_classes=ny) #4--->8
        self.genblock2 = ResBlockGenerator(GEN_SIZE*10, GEN_SIZE*8, num_classes=ny) #8--->16
        self.genblock3 = ResBlockGenerator(GEN_SIZE*8, GEN_SIZE*4, num_classes=ny) #16--->32
        self.genblock4 = ResBlockGenerator(GEN_SIZE*4, GEN_SIZE*2, num_classes=ny) #32--->64
        self.genblock5 = ResBlockGenerator(GEN_SIZE*2, GEN_SIZE, num_classes=ny) #64--->128
...
class ACGAN_Discriminator(nn.Module):
    def __init__(self, ny=100):
        super(ACGAN_Discriminator, self).__init__()

        self.discblock = nn.Sequential(
            FirstResBlockDiscriminator(channels, DISC_SIZE, stride=1, bias=True), #128--->128
            ResBlockDiscriminator(DISC_SIZE , DISC_SIZE, stride=2, bias=True), #128--->64
            ResBlockDiscriminator(DISC_SIZE , DISC_SIZE*2, stride=2, bias=True), #64--->32
            ResBlockDiscriminator(DISC_SIZE*2, DISC_SIZE*4, stride=2, bias=True), #32--->16
            ResBlockDiscriminator(DISC_SIZE*4, DISC_SIZE*8, stride=2, bias=True), #16--->8
            ResBlockDiscriminator(DISC_SIZE*8, DISC_SIZE*16, stride=2, bias=True), #8--->4
            nn.ReLU()
        )

        self.snlinear1 = nn.Linear(DISC_SIZE*16, 1)
        nn.init.xavier_uniform_(self.snlinear1.weight.data, 1.)
        self.snlinear1 = spectral_norm(self.snlinear1)

        ## aux classifier
        self.snlinear2 = nn.Linear(DISC_SIZE*16, ny)
        self.snlinear2 = spectral_norm(self.snlinear2)

    def forward(self, x, y=None, get_feature=False, act_sigmoid=False):
        features = self.discblock(x)
        features = torch.sum(features, dim=(2, 3))

        if get_feature:
            return features
        else:
            output_d = self.snlinear1(features)
            output_c = self.snlinear2(features)
            if act_sigmoid:
                return F.sigmoid(output_d), output_c
            else:
