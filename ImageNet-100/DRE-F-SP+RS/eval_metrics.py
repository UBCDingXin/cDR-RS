...
def compute_FID(PreNetFID, IMGSr, IMGSg, batch_size = 500, resize = None, normalize=False, verbose = False):
    ...
    if normalize:
        assert IMGSr.max()>=1.0 and IMGSr.max()<=255.0 and IMGSr.min()>=0
        assert IMGSg.max()>=1.0 and IMGSg.max()<=255.0 and IMGSg.min()>=0
    ...
    with torch.no_grad():
        ...
        for i in range(nr//batch_size):
            ...
            if normalize:
                imgr_tensor = imgr_tensor/255.0
                imgr_tensor = (imgr_tensor-0.5)/0.5
                assert imgr_tensor.min()>=-1 and imgr_tensor.max()<=1.0
            if resize is not None:
        ...
        for j in range(ng//batch_size):
            ...
            if normalize:
                imgg_tensor = imgg_tensor/255.0
                imgg_tensor = (imgg_tensor-0.5)/0.5
                assert imgg_tensor.min()>=-1 and imgg_tensor.max()<=1.0
            if resize is not None:
...
# Inception Scores
##############################################################################
def compute_IS(PreNetIS, IMGSg, batch_size = 500, splits=1, resize = None, normalize=False, verbose = False):
    #resize: if None, do not resize; if resize = (H,W), resize images to 3 x H x W
