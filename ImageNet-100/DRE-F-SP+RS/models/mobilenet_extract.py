...
class mobilenet_v2_extract(nn.Module):
    ...
    def __init__(self, extract_features = False, num_classes=10):
        ...
        self.conv2 = nn.Conv2d(320, 2048, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn2 = nn.BatchNorm2d(2048)

        if self.extract_features:
            self.extract_1 = nn.Sequential(
                nn.Conv2d(2048, 1024, kernel_size=3, stride=1, padding=1), #h=h/2; 8-->4
                nn.BatchNorm2d(1024),
                nn.ReLU()
            )
            self.extract_2 = nn.Sequential(
                nn.Conv2d(1024, 1024, kernel_size=3, stride=1, padding=1), #h=h; 4-->4
                nn.BatchNorm2d(1024),
                nn.ReLU()
            )
    ...
    def forward(self, x):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layers(out)
        out = F.relu(self.bn2(self.conv2(out))) #1024*8*8
        if self.extract_features:
