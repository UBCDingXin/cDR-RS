...
# training function
def train_dre(trainloader, dre_net, dre_precnn_net, netG, net_y2h, path_to_ckpt=None, do_label_norm=None, do_label_denorm=None):
    ...
    # define optimizer
    optimizer = torch.optim.Adam(dre_net.parameters(), lr = dre_lr_base, betas=(0, 0.999), weight_decay=1e-4)
    ...
    for epoch in range(dre_resume_epoch, dre_epochs):
        ...
        for batch_idx, (batch_real_images, batch_real_labels) in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                batch_fake_images = netG(z, net_y2h(do_label_norm(batch_real_labels)))
        ...
        print("DRE+{}+lambda{} Age {}: [epoch {}/{}] [train loss {}] [Time {}]".format(dre_net_name, dre_lambda, batch_real_labels[0].item(), epoch+1, dre_epochs, train_loss/(batch_idx+1), timeit.default_timer()-start_time))

        avg_train_loss.append(train_loss/(batch_idx+1))

        # save checkpoint
        if path_to_ckpt is not None and ((epoch+1) % 150 == 0 or (epoch+1)==dre_epochs):
