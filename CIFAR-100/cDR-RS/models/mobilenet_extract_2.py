...
class mobilenet_v2_extract_2(nn.Module):
    ...
    def __init__(self, num_classes=100, nc=3, img_height=32, img_width=32):
...
class mobilenet_decoder(nn.Module):
    def __init__(self):
        ...
        self.deconv = nn.Sequential(
            ## 32x32x3 = 192x4x4
            nn.ConvTranspose2d(192, 256, kernel_size=4, stride=2, padding=1), #h=2h; 8
            nn.BatchNorm2d(256),
            nn.ReLU(True),
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1), #h=h
            nn.BatchNorm2d(256),
            nn.ReLU(),

            nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1), #h=2h; 16
            nn.BatchNorm2d(128),
            nn.ReLU(True),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1), #h=h
            nn.BatchNorm2d(128),
            nn.ReLU(),

            nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1), #h=2h; 32
            nn.BatchNorm2d(64),
            nn.ReLU(True),
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1), #h=h
            nn.BatchNorm2d(64),
            nn.ReLU(),

            nn.Conv2d(64, 3, kernel_size=3, stride=1, padding=1), #h=h
            nn.Tanh()
        )

    def forward(self, input):
        input = input.view(-1, 192, 4, 4)
        x = self.deconv(input)
        return x
...
if __name__ == "__main__":
    ...
    decoder = mobilenet_decoder().cuda()
    x_hat = decoder(f)
    print(x_hat.size())
