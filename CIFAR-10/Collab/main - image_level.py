...
if args.disc_shaping:
    ...
    for i in pbar:
        ...
        batch_fake_images = netG(noise_batch, batch_labels)

        # probabilistic refinement
        proba_refine = torch.zeros(batch_fake_images.size(), dtype=torch.float32, requires_grad=False, device="cuda")
        ...
        delta_refine = torch.zeros(batch_fake_images.size(), dtype=torch.float32, requires_grad=True, device="cuda")
        ...
        for k in range(args.rollout_steps):
            optim_r.zero_grad()
            output = netD(batch_fake_images.detach() + delta_refine, batch_labels)
        ...
        # train with refined
        adv_label.fill_(0)
        output = netD((batch_fake_images+proba_refine).detach(), batch_labels)
...
def collab_sampling(given_label, netG, netD, nfake=10000, batch_size=100, verbose=True):
    ...
    while num_taken < nfake:
        ...
        batch_fake_images = netG(z, batch_labels)
        ...
        for k in range(args.rollout_steps):
            optim_r.zero_grad()
            batch_fake_images_refined = batch_fake_images.detach() + delta_refine
            batch_fake_images_refined = torch.clip(batch_fake_images_refined, min=-1, max=1)
            output = netD(batch_fake_images_refined, batch_labels)
            loss_r = criterion(output.view(-1,1), avd_label.view(-1,1))
            loss_r.backward()
            optim_r.step()
        batch_fake_images = (batch_fake_images+delta_refine).detach().cpu().numpy()
        batch_fake_images = np.clip(batch_fake_images, a_min=-1, a_max=1)
